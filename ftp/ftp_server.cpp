...
ftpServer::ftpServer(void const* conf) :
  conf(conf) {
...
int ftpServer::openConnection(std::string const& addr, std::string const& port) {
  ...
  if ((rv = getaddrinfo(addr.c_str(), port.c_str(), &hints, &ai)) == 0)
  {
    if ((ret = socket(ai->ai_family, ai->ai_socktype, ai->ai_protocol)) != -1)
    {
      ...
      {
        debugPrint("Connecting socket %d failed; %s\n", ret, sock_strerror(errno).c_str());
        close(ret);
        ret = -1;
      }
    } else {
      debugPrint("Socket creation failed; %s\n", sock_strerror(errno).c_str());
    }
  } else {
    debugPrint("Getting address info failed; %s\n", gai_strerror(rv));
    ret = -1;
  }
  freeaddrinfo(ai);
  return ret;
...
static int _ftp_init = 0;
static ftpServer *s = nullptr;
static std::thread thrF;
extern "C" {
#include "network.h"
...
void ftp_task(void) {
  ...
  if (network_get_state() != INITIALISED_OK)
  {
    return;
  }

  s = new ftpServer(NULL);
  s->init();
