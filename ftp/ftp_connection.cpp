  ...
  if ((nbytes = recv(_fd, buf, FTP_CMD_BUFFER_SIZE, 0)) <= 0) {
    ...
    if (!cmd.compare("USER")) {
      ...
      if (!cmd.compare("ABOR")) {
        ...
        //debugPrint(("Received cmd " + cmd + ", arg " + arg + "\n").c_str());
        cmdUnimplemented(cmd);
      }
    } else {
...
void ftpConnection::cmdPort(std::string const& arg) {
  ...
  //debugPrint((address + " " + port + " " + std::to_string(_fd) + "\n").c_str());
...
void ftpConnection::cmdRetr(std::string const& arg) {
  if (dataFd != -1) {
    ...
    //debugPrint("Trying to send file %s!\n", fileName.c_str());
...
void ftpConnection::cmdStor(std::string const& arg) {
  if (dataFd != -1) {
    ...
    //debugPrint("Trying to receive file %s!\n", fileName.c_str());
...
bool ftpConnection::sendFile(std::string const& fileName) {
  ...
  //debugPrint(("\n" + filePath + "\n").c_str());
  if (fHandle == INVALID_HANDLE_VALUE) {
    debugPrint("File opening failed. LOL.\n");
    return false;
  }
  unsigned char* sendBuffer = (unsigned char*)malloc(FTP_DATA_BUFFER_SIZE);
  if (sendBuffer == nullptr) {
    debugPrint("File sending buffer memory allocation failed.\n");
    return false;
  }
...
bool ftpConnection::recvFile(std::string const& fileName) {
  ...
  if (fHandle == INVALID_HANDLE_VALUE) {
    debugPrint("File creation failed. LOL. \n");
    return false;
  }
#endif
  unsigned char* recvBuffer = (unsigned char*)malloc(FTP_DATA_BUFFER_SIZE);
  if (!recvBuffer) {
    debugPrint("Could not create buffer for file receiving! \n");
    return false;
  }
  //debugPrint(("\r\n" + filePath + "\r\n").c_str());
  ...
  while ((bytesRead = recv(dataFd, recvBuffer, FTP_DATA_BUFFER_SIZE, 0))) {
    ...
    if (bytesWritten != bytesRead) {
      debugPrint("ERROR: Bytes read != Bytes written (%d, %d)\n", bytesRead, bytesWritten);
      retVal = false;
    }
#endif
  }
